 ...
 */
package cn.pku.net.db.storm.ndvr.image.analyze.harris.match;
...
import cn.pku.net.db.storm.ndvr.image.analyze.harissurf.SURFInterestPointN;
import cn.pku.net.db.storm.ndvr.image.analyze.kdtree.KDTree;
import cn.pku.net.db.storm.ndvr.image.analyze.reftype.ModifiableConst;
...
public class SurfMatchPoints {
    ...
    public static List<SurfMatch> findMatchesBBF(List<SURFInterestPointN> keys1, KDTree kd) {
        ...
        for (SURFInterestPointN sp : keys1) {
            ...
            if (Math.abs(kpN.getOrientation() - sp.getOrientation()) > ModifiableConst
    ...
    public static List<SurfMatch> filterFarMatchL(List<SurfMatch> matches, double minX, double minY) {
        ...
        for (SurfMatch m : matches) {
            ...
            double r = Math.atan((m.sp2.getY() + minY - m.sp1.getY())
                                 / (m.sp2.getX() + minX - m.sp1.getX()))
                       * 360 / (2 * Math.PI);
            if (r < 0)
                r += 90;
    ...
    public static List<SurfMatch> filterFarMatchR(List<SurfMatch> matches, double minX, double minY) {
        ...
        for (SurfMatch m : matches) {
            double r = Math.atan((m.sp1.getY() - (m.sp2.getY() + minY))
                                 / (m.sp1.getX() + minX - m.sp2.getX()))
                       * 360 / (2 * Math.PI);
