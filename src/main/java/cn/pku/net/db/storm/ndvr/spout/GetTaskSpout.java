/**
 * @Title: GetTaskSpout.java 
 * @Package cn.pku.net.db.storm.ndvr.spout 
 * @Description: get task from MongoDB and send task to EntryBolt
 * @author Jiawei Jiang    
 * @date 2014年12月27日 下午2:52:46 
        ...
        if (!completed) {
            ...
            for (TaskEntity task : taskList) {
                ...
                if (Const.STORM_CONFIG.RETRIEVAL_TASK_FLAG.equals(taskType)) {
                    ...
                    int fieldGroupingId = videoInfoList.get(0).getDuration()
                    ...
                    for (VideoInfoEntity ent1 : videoInfoList) {
                        for (VideoInfoEntity ent2 : videoInfoList) {
                            ...
                            if (Integer.parseInt(ent1.getVideoId()) < Integer.parseInt(ent2
                                ...
                                int fieldGroupingId = ent1.getDuration()
                                                      / Const.STORM_CONFIG.BOLT_DURATION_WINDOW;
                                collector.emit(new Values(taskId, taskType, fieldGroupingId,
                                    ctrlMsg));
                            }
                        }
                    }
