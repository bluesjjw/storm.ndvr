...
import cn.pku.net.db.storm.ndvr.image.analyze.search.cluster.ClusterBuilder;
import cn.pku.net.db.storm.ndvr.image.analyze.search.cluster.Clusterable;
import cn.pku.net.db.storm.ndvr.image.analyze.search.cluster.impl.Cluster;
import cn.pku.net.db.storm.ndvr.image.analyze.search.util.ClusterUtils;
import cn.pku.net.db.storm.ndvr.image.analyze.search.util.TreeUtils;
...
public class KMeansTreeNode implements Clusterable, Serializable {
    ...
    public KMeansTreeNode(float[] center, List<Clusterable> items, int branchFactor, int maxHeight,
                          int height, ClusterBuilder clusterBuilder) {
        // TODO: Something about this global variable
        if (height == maxHeight || items.size() < branchFactor || (getMeanDist(items, center) < 0)) {
            isLeafNode = true;
    ...
    public String toString() {
        return "KMeansTreeNode [isLeafNode=" + isLeafNode + ", center=" + Arrays.toString(center)
               + ", height=" + height + ", numSubItems=" + numSubItems + ", currentItems="
               + currentItems + ", id=" + id + "]";
    }
