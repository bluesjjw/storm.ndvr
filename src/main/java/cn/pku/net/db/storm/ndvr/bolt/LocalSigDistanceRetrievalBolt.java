/**
 * @Title: LocalSigDistanceBolt.java 
 * @Package cn.pku.net.db.storm.ndvr.bolt 
 * @Description: TODO
 * @author Jiawei Jiang    
 * @date 2015年1月4日 下午9:59:06 
...
/**
 * @ClassName: LocalSigDistanceBolt
 * @Description: TODO
 * @author Jiawei Jiang
 * @date 2015年1月4日 下午9:59:06
 */
public class LocalSigDistanceRetrievalBolt extends BaseBasicBolt {

  private static final Logger logger = Logger
      .getLogger(LocalSigDistanceRetrievalBolt.class);
  ...
  public void execute(Tuple input, BasicOutputCollector collector) {
    ...
    if (Const.STORM_CONFIG.RETRIEVAL_TASK_FLAG.equals(taskType)) {
      ...
      if (Const.STORM_CONFIG.IS_FILTER_AND_REFINE
        ...
        String globalSimilarVideoListStr = ctrlMsg
            .get("globalSimilarVideoList");
        Type globalSimilarVideoListType = new TypeToken<List<GlobalSimilarVideo>>() {
        }.getType();
        globalSimilarVideoList = (new Gson())
            .fromJson(globalSimilarVideoListStr, globalSimilarVideoListType);
      ...
      // 如果不采用filter-and-refine的策略,则根据视频时长选取合适的视频
        ...
        for (int duration = videoDurationWindowMin; duration <= videoDurationWindowMax; duration++) {
          ...
          // 如果cache中有对应时长的视频,则直接查询内存的Map
            ...
            comparedVideoIdSet
                .addAll(this.cachedVideoIdByDuration.get(duration));
          }
        }
      }
      // 获得query视频的局部标签
      ...
      List<List<KDFeaturePoint>> queryLocalSigs = (new Gson())
          .fromJson(queryLocalSigStr, queryLocalSigType);
      ...
      if (null == queryLocalSigs || queryLocalSigs.isEmpty()) {
        String localSimilarVideoListStr = (new Gson())
      ...
      for (String comparedVideoId : comparedVideoIdSet) {
        ...
        try {
          BufferedReader reader = new BufferedReader(
              new FileReader(comparedVideoSIFTFile));
          String line = reader.readLine();
          // 文件的一行代表一个compare视频
          if (null != line) {
            // compare视频的SIFT标签
            VideoSIFTSigEntity comparedVideoLocalSig = (new Gson())
                .fromJson(line, VideoSIFTSigEntity.class);
            ...
            for (int i = 0; i < queryLocalSigs.size(); i++) {
              ...
              for (int j = comparedFrameLeft; j <= comparedFrameRight; j++) {
                ...
                try {
                  // 比较query视频和compare视频
                  List<Match> ms = MatchKeys.findMatchesBBF(
                      queryLocalSigs.get(i),
                      comparedKeyframeSigs.get(j).getSig());
                  ms = MatchKeys.filterMore(ms);
                  // 如果找到一个相似度大于阈值的帧,则开始比较query视频的下一个帧图像
                  if ((float) ms.size() / (float) queryLocalSigs.get(i)
                      .size() >= Const.STORM_CONFIG.LOCALSIG_KEYFRAME_SIMILARITY_THRESHOLD) {
                    similarKeyframeNum++;
                    break;
                  }
                } catch (IllegalArgumentException e) {
                  logger
                      .error("ComparedVideoId: " + comparedVideoId + ", " + e);
                }
              }
            }
          ...
          logger.error("io error when read file: " + comparedVideoSIFTFilePath,
              e);
        }
      }
      ...
      logger.info("Control message size in local feature distance: " + msgLength
          + ", taskId: " + taskId);
      ...
      VideoInfoEntity queryVideoInfo2 = (new Gson())
          .fromJson(queryVideoInfoStr2, VideoInfoEntity.class);
      // 获得第二个视频的局部标签
      String queryLocalSigStr2 = ctrlMsg.get("localSignature2");
      // 第二个视频的标签,list大小为视频帧图像的数量,内层list大小为SIFT特征点的个数
      List<List<KDFeaturePoint>> queryLocalSigs2 = (new Gson())
          .fromJson(queryLocalSigStr2, queryLocalSigType);

      // 如果没有局部标签,则相似度设为0并输出
      if (null == queryLocalSigs || queryLocalSigs.isEmpty()
          || null == queryLocalSigs2 || queryLocalSigs2.isEmpty()) {
      ...
      for (int i = 0; i < queryLocalSigs.size(); i++) {
        ...
        for (int j = comparedFrameLeft; j <= comparedFrameRight; j++) {
          ...
          try {
            ...
            ms = MatchKeys.filterMore(ms);
            // 如果找到一个相似度大于阈值的帧,则开始比较query视频的下一个帧图像
            if ((float) ms.size() / (float) queryLocalSigs
                .size() >= Const.STORM_CONFIG.LOCALSIG_KEYFRAME_SIMILARITY_THRESHOLD) {
              similarKeyframeNum++;
              break;
            }
  ...
  public void declareOutputFields(OutputFieldsDeclarer declarer) {
    declarer.declare(
        new Fields("taskId", "taskType", "fieldGroupingId", "ctrlMsg"));
  }
