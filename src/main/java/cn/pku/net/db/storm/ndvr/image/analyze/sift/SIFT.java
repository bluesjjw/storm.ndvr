 ...
 */
package cn.pku.net.db.storm.ndvr.image.analyze.sift;

import java.util.ArrayList;
import java.util.List;

import cn.pku.net.db.storm.ndvr.image.analyze.sift.scale.FeaturePoint;
import cn.pku.net.db.storm.ndvr.image.analyze.sift.scale.GaussianArray;
import cn.pku.net.db.storm.ndvr.image.analyze.sift.scale.KDFeaturePoint;
import cn.pku.net.db.storm.ndvr.image.analyze.sift.scale.OctaveSpace;
import cn.pku.net.db.storm.ndvr.image.analyze.sift.scale.Pyramid;
import cn.pku.net.db.storm.ndvr.image.analyze.sift.scale.ScalePeak;
...
public class SIFT {
    ...
    public int detectFeaturesDownscaled(ImagePixelArray img, int preProcessMark, float startScale) {
        ...
        for (int on = 0; on < pyr.octaves.size(); ++on) {
            ...
            ArrayList<ScalePeak> peaksFilted = osp.filterAndLocalizePeaks(peaks, maximumEdgeRatio,
                dValueLowThresh, scaleAdjustThresh, relocationMaximum);
            ...
            ArrayList<FeaturePoint> faturePoints = osp.makeFeaturePoints(peaksFilted,
                peakRelThresh, scaleSpaceLevels, octaveSigma);
            osp.clear();
