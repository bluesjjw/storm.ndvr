...
import cn.pku.net.db.storm.ndvr.image.analyze.harris.Corner;
import cn.pku.net.db.storm.ndvr.image.analyze.harris.HarrisFast;
...
public class HarrisSurf {
     ...
     * @param matchMap
     */
    public static void geometricFilter(Map<SURFInterestPoint, SURFInterestPoint> matchMap, int w,
                                       int h) {
        if (matchMap.size() <= 1)
            return;
    ...
    private void getOrientation(SURFInterestPoint input) {
        ...
        for (ang1 = 0; ang1 < 2 * Math.PI; ang1 += 0.15f) {
            ang2 = (float) (ang1 + Math.PI / 3.0f > 2 * Math.PI ? ang1 - 5.0f * Math.PI / 3.0f
                : ang1 + Math.PI / 3.0f);
            sumX = sumY = 0;
    ...
    private float haarX(int row, int column, int s) {
        return ImageTransformUtils.BoxIntegral(mIntegralImage, row - s / 2, column, s, s / 2)
               - 1
               * ImageTransformUtils.BoxIntegral(mIntegralImage, row - s / 2, column - s / 2, s,
                   s / 2);
    }
    ...
    private float haarY(int row, int column, int s) {
        return ImageTransformUtils.BoxIntegral(mIntegralImage, row, column - s / 2, s / 2, s)
               - 1
               * ImageTransformUtils.BoxIntegral(mIntegralImage, row - s / 2, column - s / 2,
                   s / 2, s);
    }
