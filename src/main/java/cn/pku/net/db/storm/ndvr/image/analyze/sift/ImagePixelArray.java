    ...
    public ImagePixelArray doubled() {
        ...
        for (int y = 0; y < this.height - 1; y++) {
            for (int x = 0; x < this.width - 1; x++) {
                ...
                db.data[2 * (x + y * nw) + nw] = (this.data[x + y * this.width] + this.data[x
                                                                                            + (y + 1)
                                                                                            * this.width]) / 2.0f; // 下边插值

                db.data[2 * (x + y * nw) + 1] = (this.data[x + y * this.width] + this.data[x
                                                                                           + y
                                                                                           * this.width
                                                                                           + 1]) / 2.0f; // 右边插值
    ...
    public static ImagePixelArray minus(ImagePixelArray img1, ImagePixelArray img2) {
        ...
        for (int y = 0; y < min.height; y++) {
            for (int x = 0; x < min.width; x++) {
                min.data[x + y * min.width] = img1.data[x + y * img1.width]
                                              - img2.data[x + y * img1.width];
            }
        }
        return min;
    }
