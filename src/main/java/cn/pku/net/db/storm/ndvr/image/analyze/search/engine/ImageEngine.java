 ...
 */
package cn.pku.net.db.storm.ndvr.image.analyze.search.engine;
...
import cn.pku.net.db.storm.ndvr.image.analyze.search.tree.KMeansTree;
import cn.pku.net.db.storm.ndvr.image.analyze.search.util.SerializationUtils;
...
public class ImageEngine {
    ...
    public void buildWeight() {
        if (simpleInvertedFile == null)
            return;

        simpleWeightFile = new HashMap<Integer, Float>();

        Iterator<Entry<Integer, LinkedList<Integer>>> iter = simpleInvertedFile.entrySet()
            .iterator();
        while (iter.hasNext()) {
    ...
    public boolean loadIndex(String indexPath) {
        simpleInvertedFile = (Map<Integer, LinkedList<Integer>>) SerializationUtils
            .loadObject(indexPath);
    ...
    public void loadHistogram(String histogramPath) {
        simpleHistogramFile = (Map<Integer, List<Integer>>) SerializationUtils
            .loadObject(histogramPath);
    }
