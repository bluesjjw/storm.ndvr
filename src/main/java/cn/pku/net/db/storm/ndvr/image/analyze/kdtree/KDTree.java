 ...
 */
package cn.pku.net.db.storm.ndvr.image.analyze.kdtree;

import java.util.ArrayList;
import java.util.List;

import cn.pku.net.db.storm.ndvr.image.analyze.reftype.RefFloat;
import cn.pku.net.db.storm.ndvr.image.analyze.reftype.RefInt;
...
public class KDTree {
    ...
    private IKDTreeDomain nearestNeighbourI(IKDTreeDomain target, HyperRectangle hr,
                                            float maxDistSq, RefFloat resDistSq) {
    ...
    private IKDTreeDomain nearestNeighbourListBBFI(SortedLimitedList<BestEntry> best, int q,
                                                   IKDTreeDomain target, HyperRectangle hr,
                                                   int maxDistSq, RefInt resDistSq,
        ...
        if (nearerKd == null) {
            ...
            nearest = nearerKd.nearestNeighbourListBBFI(best, q, target, nearerHr, maxDistSq,
                distSq, searchHr, searchSteps);
        }

        // step 9
        if (best.size() >= q) {
            maxDistSq = best.get(q - 1).getDistSq();
        } else
            maxDistSq = Integer.MAX_VALUE;

        if (searchHr.size() > 0) {
            HREntry hre = searchHr.get(0);
            searchHr.remove(0);
        ...
        if (searchSteps.val > 0 && furtherHr.isInReach(target, (float) Math.sqrt(maxDistSq))) {
            ...
            if (furtherKd == null) {
                ...
                tempNearest = furtherKd.nearestNeighbourListBBFI(best, q, target, furtherHr,
                    maxDistSq, tempDistSq, searchHr, searchSteps);
            }
