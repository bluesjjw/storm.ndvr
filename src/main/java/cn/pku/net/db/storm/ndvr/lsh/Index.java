...
/**
 * Description: The index makes it easy to store vectors and lookup queries efficiently. For
    ...
    public static Index deserialize(HashFamily family, int numberOfHashes, int numberOfHashTables) {
        ...
        if (FileUtils.exists(serializationFile)) {
            try {
                ...
                LOG.error("Could not find class during deserialization: " + ex.getMessage());
            } catch (IOException ex) {
                LOG.error("IO exeption during during deserialization: " + ex.getMessage());
                ex.printStackTrace();
            }
        }
        return index;
