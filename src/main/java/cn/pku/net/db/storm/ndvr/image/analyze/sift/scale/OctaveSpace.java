 ...
 */
package cn.pku.net.db.storm.ndvr.image.analyze.sift.scale;

import java.util.ArrayList;

import cn.pku.net.db.storm.ndvr.image.analyze.reftype.RefFloat;
import cn.pku.net.db.storm.ndvr.image.analyze.sift.FloatArray;
import cn.pku.net.db.storm.ndvr.image.analyze.sift.ImagePixelArray;
import cn.pku.net.db.storm.ndvr.image.analyze.sift.scale.ScalePeak.LocalInfo;
...
public class OctaveSpace {
    ...
    public ArrayList<ScalePeak> filterAndLocalizePeaks(ArrayList<ScalePeak> peaks,
        ...
        for (ScalePeak peak : peaks) {
            ...
            if (isTooEdgelike(diffImags[peak.level], peak.x, peak.y, maximumEdgeRatio))
                continue;
            ...
            for (int y = 1; y < (h - 1); ++y) {
                for (int x = 1; x < (w - 1); ++x) {
                    magnitudes[s].data[y * w + x] = (float) Math
                        .sqrt(Math.pow(smoothedImgs[s].data[y * w + x + 1]
                                       - smoothedImgs[s].data[y * w + x - 1], 2.0f)
                              + Math.pow(smoothedImgs[s].data[(y + 1) * w + x]
                                         - smoothedImgs[s].data[(y - 1) * w + x], 2.0f));

                    directions[s].data[y * w + x] = (float) Math.atan2(
                        smoothedImgs[s].data[(y + 1) * w + x]
                                - smoothedImgs[s].data[(y - 1) * w + x],
                        smoothedImgs[s].data[y * w + x + 1] - smoothedImgs[s].data[y * w + x - 1]);
                }
            }
        }
    }
    ...
    public ArrayList<FeaturePoint> makeFeaturePoints(ArrayList<ScalePeak> localizedPeaks,
                                                     float peakRelThresh, int scaleCount,
                                                     float octaveSigma) {
        ArrayList<FeaturePoint> featurePoints = new ArrayList<FeaturePoint>();
        for (ScalePeak sp : localizedPeaks) {
            ArrayList<FeaturePoint> thisPointKeys = makeFeaturePoint(this.baseScale, sp,
                peakRelThresh, scaleCount, octaveSigma);
            thisPointKeys = createDescriptors(thisPointKeys, magnitudes[sp.level],
                directions[sp.level], 2.0f, 4, 8, 0.2f);
    ...
    private ArrayList<FeaturePoint> makeFeaturePoint(float imgScale, ScalePeak point,
                                                     float peakRelThresh, int scaleCount,
                                                     float octaveSigma) {
        ...
        for (int y = yMin; y < yMax; ++y) {
            for (int x = xMin; x < xMax; ++x) {
                ...
                float gaussianWeight = (float) Math
                    .exp(-((relX * relX + relY * relY) / gaussianSigmaFactor));
        ...
        interpolateOrientation(boxes[maxBox == 0 ? (36 - 1) : (maxBox - 1)], boxes[maxBox],
            boxes[(maxBox + 1) % 36], ref1);
        ...
        for (int b = 0; b < 36; ++b) {
            ...
            FeaturePoint fp = new FeaturePoint(this.smoothedImgs[point.level], point.x
                                                                               + point.local.fineX,
    ...
    private boolean interpolateOrientation(float left, float middle, float right,
                                           RefPeakValueAndDegreeCorrection ref) {
    ...
    private ArrayList<FeaturePoint> createDescriptors(ArrayList<FeaturePoint> featurePoints,
                                                      ImagePixelArray magnitude,
                                                      ImagePixelArray direction,
                                                      float considerScaleFactor, int descDim,
        ...
        for (FeaturePoint fp : featurePoints) {
            ...
            for (int y = -radius; y < radius; ++y) {
                for (int x = -radius; x < radius; ++x) {
                    ...
                    // 使该点不超出描述器的范围
                    if (yR >= (dDim05 + 0.5) || xR >= (dDim05 + 0.5) || xR <= -(dDim05 + 0.5)
                        || yR <= -(dDim05 + 0.5))
                        continue;
                    // 计算关键点和加权的点的具体x位置
                    ...
                    float dir = direction.data[currentY * direction.width + currentX]
                                - fp.orientation;
                    if (dir <= -Math.PI)
                        dir += Math.PI;
                    if (dir > Math.PI)
                    ...
                    for (int iy = 0; iy < 2; ++iy) {
                        for (int ix = 0; ix < 2; ++ix) {
                            for (int d = 0; d < 2; ++d) {
                                int idx = xIdx[ix] * fp.yDim * fp.oDim + yIdx[iy] * fp.oDim
                                          + dirIdx[d];
    ...
    private ArrayList<ScalePeak> findPeaks4ThreeLayer(ImagePixelArray below,
                                                      ImagePixelArray current,
                                                      ImagePixelArray above, int curLev,
                                                      float dogThresh) {
        ArrayList<ScalePeak> peaks = new ArrayList<ScalePeak>();
    ...
    private void checkMinMax(ImagePixelArray layer, float c, int x, int y, RefCheckMark ref,
