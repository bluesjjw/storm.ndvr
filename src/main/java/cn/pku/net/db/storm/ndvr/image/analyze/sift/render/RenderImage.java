 ...
 */
package cn.pku.net.db.storm.ndvr.image.analyze.sift.render;

import java.awt.Graphics2D;
import java.awt.image.BufferedImage;

import cn.pku.net.db.storm.ndvr.image.analyze.sift.IPixelConverter;
...
public class RenderImage {
    ...
    public float scaleWithin(int dim) {
        ...
        // 创建一个缩小后的位图
        BufferedImage bmScalaed = new BufferedImage(
            (int) (this.srcImage.getWidth() * smallestScala + 0.5),
            (int) (this.srcImage.getHeight() * smallestScala + 0.5), BufferedImage.TYPE_INT_ARGB);
    ...
    public ImagePixelArray toPixelFloatArray(IPixelConverter converter) {
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                ...
                int B = (c >> 0) & 0xFF;
                if (converter == null)
                    res.data[x + y * w] = (R + G + B) / (255.0f * 3.0f); // 默认实现直接计算，减少h*w次方法调用
                else
                    res.data[x + y * w] = converter.convert(R, G, B);
            }
        }
        return res;
    }
